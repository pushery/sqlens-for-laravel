...
final readonly class RemediationPayload
{
     ...
     */
    public const string PUBLISHED_SCHEMA = 'resources/data/schemas/remediation-payload-v1.schema.json';

    // Still 1 after the `debt_kind` field landed, and that is a decision rather than an oversight.
    //
    // The reason has MOVED once, so the old one is not left standing to be believed. It used to be
    // that no payload had ever been produced at all — true while the slot was reserved and no rule
    // filled it. Rules fill it now. What still holds is the part that matters: no payload of any
    // shape has ever been RELEASED, so no consumer has a version 1 to migrate away from, and
    // spending a version number on a document nobody received would make the first real one look
    // like a migration from something that never existed. The bump obligation is live from the
