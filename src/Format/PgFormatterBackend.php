 ...
 *
 * `pg_format` has a leading-comma mode and a line-width bound, `--comma-start` and `--wrap-limit`,
 * and neither produces the style the option names. Measured on pg_format 5.11:
 *
...
final readonly class PgFormatterBackend extends ExternalSqlFormatter
{
    ...
    protected function arguments(Dialect $dialect, FormatStyle $style): array
    {
        return [
            ...
            '--nogrouping',
            // Without `--no-rcfile`, pg_format reads `./.pg_format`, `$HOME/.pg_format` and its XDG
            // file, and every option this list does not set comes from there. Measured: a `.pg_format`
            // with `function-case=2` in the working directory turns `count(id)` into `COUNT(id)`. The
            ...
            // DML (`where ((b = 1))` becomes `WHERE (b = 1)`), and a formatter here rewrites no expression.
