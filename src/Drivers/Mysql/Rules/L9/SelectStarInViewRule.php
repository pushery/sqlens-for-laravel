...
/**
 * `SELECT *` in a view definition, which MySQL freezes into a fixed column list.
 *
 * Measured against MySQL 8.4.10: a view over `SELECT *` on a two-column table reads back as
 * ``select `t`.`a` AS `a`, `t`.`b` AS `b` from `t` `` from `information_schema.VIEWS`. Add a third column to the table and it reads back the same way — the view does not
 ...
 * - **audit reads the catalog**, where the star no longer exists — MySQL expanded it on
...
final class SelectStarInViewRule extends AbstractCatalogRule implements DeclaresJudgedObjectTypes, JudgesMigrationStatements
{
    ...
    public function judgeSchemaObject(SchemaObject $object): array
    {
        ...
        return [RuleVerdict::undetermined(
            ...
            .'here: MySQL resolved the star into a fixed set of columns when the view was created and '
    ...
    public function judgeStatement(MigrationStatementView $statement): ?RuleVerdict
    {
        ...
        return RuleVerdict::flag(
            'This view selects '.$named.', and MySQL expands that into a fixed column list when the '
