...
/**
 * An identifier that does not survive PostgreSQL unquoted.
 *
 * ## PostgreSQL folds what is not quoted, and keeps what is
 *
 * `CREATE TABLE "Orders"` and `CREATE TABLE Orders` do not make the same table. The first is
 * `Orders`; the second is folded to `orders`. Afterwards `SELECT * FROM Orders` reaches the second
 * and never the first, and a schema carrying both has two tables a reader believes are one.
 *
...
final class SnakeCaseIdentifiersRule extends AbstractCatalogRule implements DeclaresJudgedObjectTypes, JudgesMigrationStatements
{
    ...
    {
        return 'PG.L8.NAMING_SNAKE_CASE';
    }
    ...
    private function sentence(string $type, string $name): string
    {
        return 'The '.$type.' `'.$name.'` is not snake_case: '.SnakeCaseIdentifiers::reason($name)
            .'. On PostgreSQL every reference to it must be quoted, forever.';
    }
}
