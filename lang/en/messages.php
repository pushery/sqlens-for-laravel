...
return [
    ...
    'catalog' => [
        ...
        'skip' => [
            ...
            'insufficient_privilege' => 'The connecting role lacks the privilege to read :reference, so its objects are missing from this reading. Grant the reading role what it needs — SELECT on the relation, or a role such as pg_read_all_settings or pg_monitor for the server catalogs — and the reading is complete. On a managed database that grant may not be available to you at all, which is the ordinary case there; then this names the part of the audit that cannot be made on that platform.',
            'not_understood' => 'SQLens read :reference, but part of it did not arrive in a form a rule can reason about — an expression recorded beside the index rather than among its key positions, a condition whose shape the reading does not recognize. It is reported rather than guessed at: a wrong answer about an index costs more than a missing one.',
            'not_comparable' => 'SQLens read :reference completely, and it lies outside a comparison this package deliberately does not make — a b-tree under a non-default operator class, a partial index beside another condition whose implication is not decided here. Nothing went unread, and nothing is owed: the rule that draws the boundary names it on its page.',
    ...
    'remediation' => [
        ...
        'no_safe_sequence' => [
            'type_mismatch_composite_key' => 'There is no standard sequence for this, because the key is composite and the columns of a composite key cannot be widened one at a time without deciding an order. Each ALTER takes its own exclusive lock, including on the columns that were already right, and which one goes first depends on what reads the table while it happens — which this package cannot see. Widen them together, in one migration, with that question answered.',
            'type_mismatch_parent_is_narrower' => 'There is no standard sequence for this, and the obvious one points the wrong way. The narrower side here is the key being REFERENCED, not the column referencing it, so matching them means widening the parent — narrowing the child to fit would cement the smaller ceiling and fail on the first value above it. Widening a referenced key moves every child that points at it, and that set is a schema decision rather than a statement.',
            'type_mismatch_not_an_integer_pair' => 'There is no standard sequence for this, because the two sides are not two widths of one number. A timestamp against a datetime, a varchar against a char: each pair has its own correct answer, and it depends on which side is telling the truth about the data — which is a question about your application, not about the catalog.',
            ...
            'no_primary_key_live_table' => 'There is no standard sequence for this, because putting a key on this table needs a candidate that is unique AND NOT NULL across every row already in it — a fact about your data, which SQLens never reads. This is the harder of the two keyless cases: the table is in your database, so it is populated, and the empty-table window a fresh CREATE TABLE has is gone. If a natural candidate exists, add it as the primary key in a migration of its own and expect a rebuild of every row. If none does, the route is the staged swap: add a surrogate column, backfill it in batches, then promote it — never a one-line ALTER on a table this size. SQLens does not choose for you, because on InnoDB the primary key IS the physical row order.',
            ...
            'money_in_a_binary_float' => 'There is no standard sequence for this, because the TARGET is settled and the PATH is a decision about your data. numeric(19, 4) is the answer — the rule says so and means it — but getting a populated column there is not one ALTER: the existing values were stored as binary floats, so converting them decides how each one rounds, and that is a decision about money nobody but you may make. On a table small enough to rewrite in a window, one ALTER after you have checked the rounding is fine; on a large one it is the staged swap — add the numeric column, backfill it in batches, then promote it. SQLens does not pick, because the wrong pick here is silently wrong.',
        ...
        'expand_contract' => [
            ...
            'swap_the_key' => 'Move the foreign key across: drop the constraint, point it at the new column, add it back. One migration, because a table with the constraint on neither column accepts rows that reference nothing — and the window where that is true is the whole risk of this step. Add it back NOT VALID and validate it separately if the table is large enough that the check itself is the problem; that is its own strategy.',
            'precondition' => [
                ...
                'in_place_rewrites_the_table' => 'You have chosen between the sequence below and the one statement that replaces it. `ALTER TABLE … ALTER COLUMN … TYPE bigint` reaches the same end state in a single migration, and on PostgreSQL it rewrites every row and holds ACCESS EXCLUSIVE while it does — readers blocked, not only writers, for as long as the table takes. On a small table that is seconds and the sequence below is overkill. On a large one it is an outage, and the size at which that flips is a fact about your table that nothing here can read.',
                'the_parent_is_the_wider_side' => 'The column being widened is the one that REFERENCES, and the key it points at is already the wider of the two. Widening the other direction — narrowing the referenced key to match — would cement the smaller ceiling and fail on the first value above it.',
            ],
        ],
        'reindex_before_refresh' => [
            'find_dependents' => 'List the indexes that sort under this collation. The finding names the collation, not the indexes, because the two are a query apart and guessing the set is how one gets missed — an index left with the old sort order keeps answering the wrong way after everything else is fixed.',
            'reindex' => 'Rebuild each of those indexes, one statement per index, outside a transaction. CONCURRENTLY keeps writers going while it runs; without it the table is locked for the rebuild. This is the step that actually repairs the sort order — everything else is bookkeeping around it.',
            'refresh_version' => 'ONLY NOW record the collation version as current. Doing this first is worse than doing nothing: it silences the warning while every index is still sorted by rules the server no longer uses, and the one signal that something is wrong is gone.',
            'verification' => 'Run sqlens:audit again — the collation must no longer report a version mismatch, and no index may be left INVALID by an interrupted rebuild.',
            'precondition' => [
                'not_a_replica' => 'This is the primary. A rebuild on a replica is not possible and the version a replica records follows its primary.',
                'interruption_is_survivable' => 'An interrupted concurrent rebuild leaves the old index in place and marks the new one invalid; the sequence can be run again. Know that before starting it on a large index.',
            ],
        ],
        'concurrently' => [
